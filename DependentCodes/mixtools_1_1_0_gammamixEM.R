gammamixEM <-
  function (x,
    ...
    while (diff > epsilon && iter < maxit) {
      ...
      # yjf. invoke nlm package for optimizing
      out = try(suppressWarnings(nlm(
        gamma.ll,
        p = theta,
        lambda = lambda.hat,
        k = k,
      ...
      if (class(out) == "try-error") {
        ...
        new.obs.ll <- sum(log(apply(
          dens(lambda.hat, theta.hat, k), 1, sum
        )))
