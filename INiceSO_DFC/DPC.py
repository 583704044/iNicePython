...
#
#     '''将各个不同分布的数据合并在一起'''
#     #    ,data2, data6, data4 ,, data11, data12, data9, data10, data5, data3
#     data = data6.append([data8, data7])  #  , data6, data7, data8, data9, data10, data11, data12
...
#                          data36, data37, data38, data39, data40, data41, data42, data43, data44, data45, data46, data47,
...
def DPC(data, t):
    ...
    threhold_of_density = (np.max(density) + np.min(density)) / 9  # +np.mean(density)/2
    threhold_of_sigma = (np.max(sigma) + np.min(sigma)) / 14
    ...
    for i in range(len(center_Point_Subscript)):  # 画中心
        centers.append(data[center_Point_Subscript[i]])
    ...
    '''为画图而附加部分:将点分配到距离它最近的中心点所属的类中'''
    # 首先计算各个点与中心点的距离
    # 然后将点分配到与之距离最近的中心点所属的类中
    # 然后画图

    dis = EuclideanDistances(data, np.array(centers))
    print("disdisdisdisdis:", DataFrame(dis))
...
if __name__=="__main__":
    ...
    #     center_Point_Subscript = DPC(data, 0.2)
    #     # print(center_Point_Subscript)
    #     end = time.process_time()
    #     runtime = end - start
    #     sumTime = sumTime + runtime
    #     print("第" + str(i) + "次运行时间为：" + str(runtime))
    #     print("final_num_of_cluster: " + str(len(center_Point_Subscript)))
    #
    ...
    # print(data.type)
    # data = [[1,2,3],[2,4,3],[4,2,5]]
    # data = [[0.21915236 , 0.41920639] [0.23534144 , 0.40826741] [0.1304452 ,0.42549228] [0.17824839 ,0.38479111]  [0.20902388 ,0.41262342]]
    data = np.array(data)
    print(type(data))
    # print(data)
    ...
    print("10次DPC的平均运行时间为：" + str(sumTime / 10))
