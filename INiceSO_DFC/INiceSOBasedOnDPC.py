...
#     '''将各个不同分布的数据合并在一起'''
#     #
#     data = data1.append([data8, data7, data5, data3, data4,data2, data6, data9, data10, data11, data12])  # , data8 , data6, data7, data8, data9, data10, data11, data12
...
def I_niceSO(dataset):
    ...
    for i in range(len_of_counts_of_each_component):
        ...
        # 画图观察每个类中对应数据的距离分布情况
        # plt.hist(dis_Based_On_Best_ViewPoint_Of_Class_i, bins=40, normed=0, facecolor="blue", edgecolor="black", alpha=0.7)
        # plt.xlabel("区间")
        # plt.ylabel("频数/频率")
        # plt.title("频数/频率分布直方图")
        # plt.show()
    plt.show()
        #'''对component_i中的所有数据进行DPC'''
        ...
        #
        # '''使用density peaks cluster(DPC)方法找出各个类中包含的所以中心点'''
        # rawdata_in_component_i = np.array(rawdata_in_component_i)
        # DPC(rawdata_in_component_i, 0.2)
...
if __name__ == '__main__':
    ...
    # np.savetxt('50.csv', data, delimiter=',')
    # print("保存成功！")

    '''读取人工生成的数据'''
    data = np.loadtxt(open("50.csv", "rb"), delimiter=",", skiprows=0)
    print("读取成功！")
    print(type(data))
    data = DataFrame(data)
    ...
    if data.shape[1] == 2:
        plt.plot(data.iloc[:, 0], data.iloc[:, 1], "x")  #
        # viewPoint = [[0.06, 0.41]] # 15个簇
        # viewPoint = [[0.6, 0.08]]  # 20个簇，适用于30个簇
        plt.plot(0.28, 0.78, 'g*')  # 40个簇
        plt.xlabel("1st Dimension")
        plt.ylabel("2nd Dimension")
        # plt.title("normalized data")
        plt.show()
    ...
    for i in range(3):
        ...
        if data.shape[1] == 2:
            ...
            # plt.clf()
            for i in range(len(index_of_clusters_in_rawdata)):
                plt.plot(data.iloc[index_of_clusters_in_rawdata[i], 0], data.iloc[index_of_clusters_in_rawdata[i], 1], 'ro')
                # plt.plot(initial_centers[i], lw=1, label='cluster_'+str(i))
    ...
    loc_of_viewPoint = loc_of_viewPoint.tolist()
    plt.plot(data.iloc[:, 0], data.iloc[:, 1], 'x')
    plt.plot(loc_of_viewPoint[0], loc_of_viewPoint[1], 'b*')
    for i in range(len(index_of_clusters_in_rawdata)):
        plt.plot(data.iloc[index_of_clusters_in_rawdata[i],0],data.iloc[index_of_clusters_in_rawdata[i],1], 'ro')
    plt.xlabel('x-axis')
    plt.ylabel('y-axis')
    plt.title('cluster centers and data based on DPC')
    plt.show()
