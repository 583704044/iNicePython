...
def I_niceSO(dataset):
    ...
    for i in range(len_of_counts_of_each_component):
        ...
        k_of_KNN = math.floor(1.0*rawdata_in_component_i.shape[0]) #KNN的k值
    ...
    # plt.xlabel("Normalized Distance Value")
    # plt.ylabel("Density")
    # plt.title("Component distribution of GMM from observer")
    # plt.show()
...
if __name__ == '__main__':
    ...
    # data = np.array(data)
    data = np.loadtxt(open("45.csv", "rb"), delimiter=",", skiprows=0)
    print("读取成功！")
    print(type(data))
    data = DataFrame(data)
    ...
    if data.shape[1] == 2:
        plt.plot(data.iloc[:, 0], data.iloc[:, 1], "x")  #
        # plt.plot(0.06, 0.41, 'g*') # 15个簇
        plt.plot(0.6, 0.08, 'g*') # 20个簇，25， 适用于30个簇
        # plt.plot(0.52, 0.81, 'g*') # 35，40
        # viewPoint = [[0.28, 0.78]]  # 45个簇
        # viewPoint = [[0.31, 0.56]]  # 50个簇
        plt.xlabel("1st Dimension")
        plt.ylabel("2nd Dimension")
