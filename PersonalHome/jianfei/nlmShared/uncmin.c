 ...
 */
    ...
    if (*ndigit == 0) {
        ...
        *ndigit = (int) (-log10(epsm)); //machine epsilon
 ...
 *	itrmcd	    <--	 termination code
 *	a(n,n)	     --> workspace for hessian (or estimate)  //allocated in nlm
 ...
 *	itncnt	     current iteration, k  {{was `internal'}}
 //wrk = (double*)R_alloc(8*n, sizeof(double));   in nlm(...)
 //wrk, wrk + nr,
 ...
 */
    ...
    *itncnt = 0;
    ...
    for (i = 0; i < n; ++i)
        p[i] = 0.;  //workspace for step

    rnf = Rexp10(-(double) ndigit);   //ndigit, default=12
    rnf = fmax2(rnf, epsm);         //epsm = DBL_EPSILON
    analtl = sqrt(rnf);
    ...
    *itrmcd = opt_stop(n, x, f, g, wrk1, *itncnt, &icscmx,
    ...
    if (iexp) {
         ...
         * hessian will be obtained by secant(割线) updates.	get initial hessian.*/
 ...
 */
    ...
    //       double *xpls, double *fpls, double *gpls, int *itrmcd,
    //       double *a,
    // wrk:      double *udiag, double *g, double *p, double *sx,
    //       double *wrk0, double *wrk1, double *wrk2, double *wrk3, int *itncnt)

    //r_nlm.c nlm:  wrk = (double*)R_alloc(8*n, sizeof(double));
